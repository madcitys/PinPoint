          ...
          <div v-else class="alerts-list">
            ...
            >
              ...
              <div class="alert-actions">
                ...
                <button
                  v-if="isLostAlert(alert) && !alert.isRead"
                  class="alert-action read"
                  :disabled="Boolean(actionBusy[alert.id])"
                  @click.stop="markAlertAsRead(alert)"
                >
                  {{ actionBusy[alert.id] ? 'Saving...' : 'Mark read' }}
                </button>
              </div>
            </article>
...
import {
  fetchAlertResponse,
  flagAlertForReview as submitAlertReview,
  markAlertRead as submitMarkAlertRead,
...
const unreadCount = computed(() => alertSummary.value.unread ?? alerts.value.filter((alert) => !alert.isRead).length);
...
function isLostAlert(alert) {
  return alert.meta?.notificationStatus === 'lost';
}
...
async function markAlertAsRead(alert) {
  ...
  try {
    await submitMarkAlertRead(alert.id, alert.version);
    alerts.value = alerts.value.map((entry) => (
      entry.id === alert.id ? { ...entry, isRead: true } : entry
    ));
    emitDataEvent(DATA_EVENTS.notificationsRefresh);
    await refreshAlerts();
  } catch (error) {
