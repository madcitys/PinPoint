    ...
    <div class="card">
      ...
      <div class="stats-grid">
        <article class="stat-card">
          ...
          <strong class="stat-value">{{ notificationsSummary.total }}</strong>
        </article>
        ...
        <article class="stat-card">
          ...
          <strong class="stat-value">{{ notificationsSummary.thisWeek }}</strong>
        </article>
      </div>
          ...
          <div class="date-filter">
            ...
            <div class="date-filter-row">
              <button ref="dateTrigger" class="btn date-trigger" @click="openDateFilter">
                ...
                <span class="date-trigger-text">{{ selectedDateRangeText || 'Pick date range' }}</span>
              </button>
              ...
              <div ref="filterAnchor" class="filter-anchor">
                <button class="btn filter-btn" :class="{ active: activeFilter !== 'all' }" @click.stop="showFilterPopup = !showFilterPopup">
                ...
                <div v-if="showFilterPopup" class="filter-popup" @click.stop>
                  ...
                  <div class="filter-group single">
                    <label><input type="radio" v-model="activeFilter" value="all" /> All</label>
                    <label><input type="radio" v-model="activeFilter" value="warning" /> Warning</label>
                    <label><input type="radio" v-model="activeFilter" value="critical" /> Critical</label>
                  </div>

                  <div class="filter-actions">
                    <button class="btn popup-clear" @click="activeFilter = 'all'">Clear</button>
      ...
      <div v-if="selectedAlertIds.length" class="bulk-actions">
        ...
        <button class="btn" :disabled="bulkBusy" @click="runBulkAction('acknowledge')">Acknowledge</button>
        <button class="btn" :disabled="bulkBusy" @click="runBulkAction('assign')">Assign to me</button>
        <button class="btn subtle" :disabled="bulkBusy" @click="runBulkAction('archive')">Archive</button>
      </div>
          ...
          <tbody>
            ...
            >
              ...
              <td class="actions-cell">
                <button class="table-action" type="button" @click.stop="openReportPage(alert)">Reports</button>
                <button
                  v-if="alert.meta?.notificationStatus === 'lost' && !alert.isRead"
                  class="table-action warn"
                  type="button"
                  :disabled="actionBusyId === alert.id"
                  @click.stop="markLostAlertRead(alert)"
                >
                  {{ actionBusyId === alert.id ? 'Saving...' : 'Mark read' }}
                </button>
              </td>
            </tr>
          </tbody>
...
import { bulkUpdateAlerts, fetchAlertResponse, markAlertRead } from '../lib/alerts';
...
const stateTabs = computed(() => [
  ...
  { value: 'resolved', label: `Resolved ${serverSummary.value.resolved ?? 0}` },
  { value: 'archived', label: `Archived ${serverSummary.value.archived ?? 0}` },
]);
...
async function markLostAlertRead(alert) {
  actionBusyId.value = alert.id;

  try {
    await markAlertRead(alert.id, alert.version);
    await loadAlerts();
  } finally {
...
async function runBulkAction(action) {
  ...
  try {
    if (action === 'acknowledge') {
      await bulkUpdateAlerts(selectedAlertIds.value, 'acknowledge');
    } else if (action === 'assign') {
      await bulkUpdateAlerts(selectedAlertIds.value, 'assign', currentUser?.id);
    } else if (action === 'archive') {
      await bulkUpdateAlerts(selectedAlertIds.value, 'archive');
    }
...
watch(
  () => route.query.filter,
  (nextFilter) => {
    activeFilter.value = typeof nextFilter === 'string' ? nextFilter : 'all';
  },
);
