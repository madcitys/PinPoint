...
create table if not exists public.retail_orders (
  ...
  retailer text not null default 'Retail Customer',
  payment_status text not null default 'pending' check (payment_status in ('paid', 'pending', 'delayed', 'partial')),
  payment_terms text default 'N/A',
  due_date date,
  channel text default 'email' check (channel in ('viber', 'whatsapp', 'email', 'phone')),
  cancel_reason text,
  created_at timestamptz default now(),
...
create index if not exists idx_parcel_history_parcel on public.parcel_history(parcel_id);
create index if not exists idx_parcel_history_created_at on public.parcel_history(created_at desc);
create index if not exists idx_retail_orders_status on public.retail_orders(status);
create index if not exists idx_retail_orders_payment_status on public.retail_orders(payment_status);
create index if not exists idx_retail_orders_due_date on public.retail_orders(due_date);
create index if not exists idx_retail_orders_updated_at on public.retail_orders(updated_at desc);
...
create or replace trigger trg_retail_orders_updated_at
