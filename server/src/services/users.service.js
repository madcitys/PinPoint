...
async function createUniqueUserId() {
  const { data, error } = await supabase.from('app_users').select('user_id').like('user_id', 'PP-%');
  throwIfSupabaseError(error);

  const usedNumbers = new Set(
    (data ?? [])
      .map((row) => row.user_id)
      .map((userId) => /^PP-(\d{6})$/.exec(userId)?.[1])
      .filter(Boolean)
      .map((value) => Number.parseInt(value, 10))
  );

  for (let nextNumber = 1; nextNumber <= usedNumbers.size + 1000; nextNumber += 1) {
    if (usedNumbers.has(nextNumber)) {
      continue;
    }

    const candidate = `PP-${String(nextNumber).padStart(6, '0')}`;
...
export async function listUsers({ search, statuses, roles, page, pageSize }) {
