...
const REPORT_COLUMNS = 'id,parcel_id,tracking_number,remarks,status,reported_at,resolved_at,updated_at,reported_by(id,first_name,last_name)';

function formatReporterName(reportedBy) {
  if (!reportedBy) return null;
  const firstName = reportedBy.first_name?.trim() ?? '';
  const lastName = reportedBy.last_name?.trim() ?? '';
...
function mapReport(row) {
  ...
  return {
    ...
    reportedBy: formatReporterName(row.reported_by),
  };
}
...
export async function deleteReport({ id, actor }) {
  ...
  const { error } = await supabase.from('parcel_reports').delete().eq('id', id);
  throwIfSupabaseError(error);

  await createActivityLog({
    parcelId: report.parcelId,
    trackingNumber: report.trackingNumber,
    action: `Report removed: ${summarizeText(report.remarks)}.`,
    category: 'report',
    actor,
  });
}
