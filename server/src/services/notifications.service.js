...
const ACTIVE_ALERT_STATES = ['new', 'read', 'assigned'];
...
function applyListFilters(query, filters) {
  ...
  } else if (filters.severityIn) {
    next = next.in('type', filters.severityIn);
  }
  ...
  if (filters.unassigned && !filters.assigned) {
...
async function buildNotificationSummary(filters) {
  ...
  const [total, active, unread, archived, resolved, warning, critical, assignedToMe, thisWeek] = await Promise.all([
    ...
    filters.currentUser ? countAlerts({ active: true, assignedTo: filters.currentUser.id }) : Promise.resolve(0),
...
export async function listNotifications({
  ...
  if (filterSet.has('warning')) severityFilters.push('warning');
  if (filterSet.has('critical')) severityFilters.push('critical');

  const filters = {
    severity: severityFilters.length === 1 ? severityFilters[0] : undefined,
    severityIn: severityFilters.length > 1 ? severityFilters : undefined,
    assigned: filterSet.has('assigned'),
    unassigned: filterSet.has('unassigned'),
    search,
    state: state ?? 'active',
    page: safePage,
  ...
  const resolvedMode = filters.state === 'resolved';

  if (resolvedMode) {
    query = query.order('resolved_at', { ascending: false }).order('created_at', { ascending: false });
  } else {
    query = query.order('days_pending', { ascending: false }).order('created_at', { ascending: false });
  }

  const resultPageSize = resolvedMode ? 20 : safePageSize;
  const resultPage = resolvedMode ? 1 : safePage;
  const from = (resultPage - 1) * resultPageSize;
  const to = from + resultPageSize - 1;
  ...
  const total = resolvedMode ? Math.min(count ?? 0, 20) : count ?? 0;

  return {
    data: (data ?? []).map(mapAlert),
    meta: {
      page: resolvedMode ? 1 : safePage,
      pageSize: resultPageSize,
      total,
      totalPages: resolvedMode ? 1 : Math.max(1, Math.ceil(total / resultPageSize)),
    },
    summary,
  };
}
...
export async function markNotificationUnread({ id, actor, expectedVersion }) {
  const alert = await getAlertRowById(id);
  assertActiveAlert(alert, 'marked as unread');
...
export async function unassignNotification({ id, actor, expectedVersion }) {
  const alert = await getAlertRowById(id);
  assertActiveAlert(alert, 'unassigned');

  if (!alert.assigned_to) {
    return mapAlert(alert);
  }

  if (alert.assigned_to !== actor?.id) {
    throw new HttpError(403, 'You can only unassign notifications assigned to you.');
  }
...
export async function flagNotificationForReview({ id, actor, expectedVersion }) {
  ...
  // Log the event but DO NOT resolve the alert - it should remain visible in All Notifications
  await insertNotificationEvent({
    alertId: alert.id,
    ...
    previousState: alert.state,
    nextState: alert.state, // State doesn't change
    metadata: {
  ...
  // Return the alert as-is (not resolved)
  return {
...
export async function bulkUpdateNotifications({ ids, action, actor, assigneeId }) {
  ...
  for (const id of uniqueIds) {
    ...
    if (action === 'unassign') {
      results.push(await unassignNotification({ id, actor }));
      continue;
    }
