...
import { syncAlerts } from './notifications.service.js';

const PARCEL_COLUMNS = 'id,tracking_number,platform,status,date_added,outbound_date,created_at,updated_at';

const SCAN_STATUS_FLOW = ['Pending', 'Outbound', 'Cancelled', 'Returned', 'Double Waybill', 'Missing', 'Lost'];
...
const RESET_OUTBOUND_DATE_STATUSES = ['Pending', 'Cancelled', 'Double Waybill', 'Missing', 'Lost'];
const RESET_OUTBOUND_DATE_ORIGINS = ['Outbound', 'Returned'];
...
function shouldResetOutboundDate(beforeStatus, nextStatus) {
  return RESET_OUTBOUND_DATE_ORIGINS.includes(beforeStatus) && RESET_OUTBOUND_DATE_STATUSES.includes(nextStatus);
}

async function syncAutomaticParcelStatuses() {
  const lostCutoff = dateDaysAgo(LOST_THRESHOLD_DAYS);
  const missingCutoff = dateDaysAgo(MISSING_THRESHOLD_DAYS);

  const [{ error: lostError }, { error: missingError }] = await Promise.all([
    supabase
      .from('parcels')
      .update({ status: 'Lost' })
      .in('status', ['Pending', 'Missing'])
      .lte('date_added', lostCutoff),
    supabase
      .from('parcels')
      .update({ status: 'Missing' })
      .eq('status', 'Pending')
      .lte('date_added', missingCutoff)
      .gt('date_added', lostCutoff),
  ]);

  throwIfSupabaseError(lostError);
  throwIfSupabaseError(missingError);

  // Refresh alerts for parcels that may have changed status
  await syncAlerts({ force: true });
}
...
export async function listParcels({ search, statuses, platforms, dateFrom, dateTo, page, pageSize }) {
  await syncAutomaticParcelStatuses();
...
export async function updateParcel({ id, platform, status, outboundDate, outboundDateProvided, actor }) {
  ...
  } else if (shouldResetOutboundDate(before.status, nextStatus)) {
    nextOutboundDate = null;
  }
...
export async function scanParcelByTrackingNumber({ trackingNumber, actor }) {
  ...
  if (nextStatus === 'Outbound' && !before.outboundDate) {
    ...
    updatePayload.outbound_date = null;
  }
