...
import { optionalEnum, optionalString, parseCsvFilter, parsePagination, requirePassword, requireString } from '../utils/validation.js';
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const roles = parseCsvFilter(req.query.role, USER_ROLES, 'role');
    const { page, pageSize } = parsePagination(req.query);

    const [usersResult, summary, registrationCode] = await Promise.all([
      listUsers({ search, statuses, roles, page, pageSize }),
      getUserStatusSummary(),
      getRegistrationCodeMeta(),
    ]);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const role = optionalEnum(req.body.role, 'role', USER_ROLES) ?? 'staff';
    const status = optionalEnum(req.body.status, 'status', USER_STATUSES) ?? 'active';
    ...
    if (status === 'inactive' && !reason) {
      throw new HttpError(400, 'A reason is required when creating an inactive user.');
    }
    ...
    await createActivityLog({
      category: 'user',
      action: `New ${user.role} account created for ${user.fullName}.`,
      actor: req.user,
    });
