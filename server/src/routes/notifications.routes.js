...
const router = Router();
const LIST_STATES = ['active', 'all', 'mine', 'assigned', 'resolved', 'archived', 'new', 'read'];
const BULK_ACTIONS = ['archive', 'assign', 'unassign', 'mark_read'];
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const rawFilters = Array.isArray(req.query.filter)
      ? req.query.filter.map(String)
      : req.query.filter
      ? String(req.query.filter).split(',')
      : [];
    const filters = rawFilters
      .map((value) => String(value).trim().toLowerCase())
      .filter((value) => ['warning', 'critical', 'assigned', 'unassigned'].includes(value));
...
router.post(
  '/:id/unread',
  asyncHandler(async (req, res) => {
    const data = await markNotificationUnread({ id: req.params.id, actor: req.user, expectedVersion: req.body?.expectedVersion });
    res.json({ data });
  }),
);
